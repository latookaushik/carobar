...
export const POST = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    const { account_number, bank_name, bank_branch, currency, description, is_default, is_active }: BankInput = validationResult.data;
...
export const PUT = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    // Parse request body
    const body = await request.json();
    const { account_number, bank_name, bank_branch, currency, description, is_default, is_active }: BankInput = body;
