...
export const GET = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    logDebug(`Fetching COA for company: ${companyId} with search: ${search}, page: ${page}, pageSize: ${pageSize}`);
    ...
    logError(`Error fetching chart of accounts: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
export const POST = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    const { account_code, account_name, account_type, description, is_active }: ChartOfAccountInput = validationResult.data;
    ...
    logError(`Error creating chart of account: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return createErrorResponse(
...
export const PUT = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    if (!oldCode) {
      return createErrorResponse(
        'Original account code is required',
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    const { account_code, account_name, account_type, description, is_active } = validationResult.data;
    ...
    logError(`Error updating chart of account: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return createErrorResponse(
...
export const DELETE = withUser(async (request: NextRequest) => {
  ...
  try {
    ...
    logError(`Error deleting chart of account: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return createErrorResponse(
